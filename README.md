...
- **安全加固** — 无默认密码、启动时哈希凭据、登录频率限制、可配置 CORS、XSS 防护、服务名注入防护
...
| `MC_ALLOWED_ORIGIN` | 可选 | CORS 允许的源，留空则拒绝跨域。生产环境设为具体域名，本地开发可设 `*` |
| `MC_SKIP_PROCS` | 可选 | 跳过的端口监听进程名（逗号分隔），默认 `sshd,systemd,...` 等系统进程 |
| `MC_WEBHOOK_URL` | 可选 | Webhook 推送地址，留空则关闭告警 |
| `MC_WEBHOOK_TYPE` | 可选 | Webhook 类型：`feishu` / `dingtalk` / `wecom` / `custom`，默认 `feishu` |
| `MC_WATCH_INTERVAL` | 可选 | 健康检查轮询间隔（秒），默认 `60` |
...
```bash
# 1. 复制项目文件（不含 .env / config.json 中的机器专属配置）
...
- 设置固定 `MC_SESSION_SECRET`，避免重启后用户被强制退出
